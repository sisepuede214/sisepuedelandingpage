...
When a known profile returns with updated **`source`**, **`signup_phase`**, or **`language`** (client calls this from **`app/components/SignupGate.tsx`** on the home page, founders waitlist, **`/[locale]/mundial-5k`** with `source=mundial_5K`, or **`/[locale]/photos`** with `source=la_bandera_austin`):
